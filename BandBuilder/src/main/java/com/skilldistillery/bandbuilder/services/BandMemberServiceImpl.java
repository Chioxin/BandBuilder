...
public class BandMemberServiceImpl implements BandMemberService {
	...
	@Override
	public BandMember createBandMember(BandMember bandMember) {
		
		if (bandMember.getInstrument() != null) {
			Optional<Instrument> optInstrument = instrumentRepo.findById(bandMember.getInstrument().getId());
			if (optInstrument.isPresent()) {
				bandMember.setInstrument(optInstrument.get());
			}			
		}
		
		if (bandMember.getBand() != null) {
			Optional<Band> optBand = bandRepo.findById(bandMember.getBand().getId());
			if (optBand.isPresent()) {
				bandMember.setBand(optBand.get());
			}
		}

		if (bandMember.getProfile() != null) {
			Optional<Profile> optBand = profileRepo.findById(bandMember.getProfile().getId());
			if (optBand.isPresent()) {
				bandMember.setProfile(optBand.get());
			}
		}
		
		return bandMemberRepo.saveAndFlush(bandMember);
	}
...
//		
//		instrument = instrumentRepo.saveAndFlush(instrument);
//		
//		// Build Profile
